...
describe('Plan Steps Queue Worker', () => {
	let env: Env;
	...
	describe('HTTP Method Validation', () => {
		it('should accept POST requests', async () => {
			...
			expect(env.PLAN_STEPS_QUEUE.send).toHaveBeenCalledWith(validPayload);
		});
		...
		it('should reject GET requests with 405', async () => {
			...
			expect(env.PLAN_STEPS_QUEUE.send).not.toHaveBeenCalled();
		});
	...
	describe('Payload Validation', () => {
		...
		it('should set default retryCount to 0 if not provided', async () => {
			...
			expect(response.status).toBe(201);
			expect(env.PLAN_STEPS_QUEUE.send).toHaveBeenCalledWith(expect.objectContaining({ retryCount: 0 }));
		});
	});

	describe('Queue Operations', () => {
		it('should handle queue send failures', async () => {
			env.PLAN_STEPS_QUEUE.send = vi.fn().mockRejectedValue(new Error('Queue error'));
